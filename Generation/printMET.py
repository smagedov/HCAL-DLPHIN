...
def main():
    parser = OptionParser()
    parser.add_option('-i', '--inputPath', default = 'GGToHtautau_13TeV_pythia8_cff_step2.root', action = 'store', type = 'string')
    (options, args) = parser.parse_args(sys.argv)
    inputPath = options.inputPath

    inputPath2 = 'GGToHtautau_13TeV_pythia8_cff.root'
	...
	metpx = []
	metpy = []
	metpt, metpx, metpy = count(inputPath, metpt, metpx, metpy)
        rawmetpt = []
        rawmetpx = []
        rawmetpy = []
        rawmetpt, rawmetpx, rawmetpy = countraw(inputPath2, rawmetpt, rawmetpx, rawmetpy)

    metpt = np.asarray(metpt)
    metpy = np.asarray(metpy)
    metpx = np.asarray(metpx)
    genmetpt = np.asarray(rawmetpt)
    genmetpx = np.asarray(rawmetpx)
    genmetpy = np.asarray(rawmetpy)
...
##____________________________________________________________________________||
def count(inputPath, metpt, metpx, metpy):
    ...
    return metpt, metpx, metpy

def countraw(inputPath2, metpt, metpx, metpy):
    ...
    for event in events:
        ...
        event.getByLabel(("genMetTrue", "", "SIM"), handleGenMETs)
        met = handleGenMETs.product().front()
